...
export class GenAILiveClient {
  // FIX: Use an internal EventEmitter instance to handle events.
  ...
  public async connect(config: LiveConnectConfig): Promise<boolean> {
    ...
    console.log('🔌 Connecting to Gemini Live API...', { model: this.model, config });
    ...
    try {
      ...
      console.error('Error details:', { message: e?.message, stack: e?.stack, config });
  ...
  public send(parts: Part | Part[], turnComplete: boolean = true) {
    if (this._status !== 'connected' || !this.session) {
      // FIX: Changed this.emit to this.emitter.emit to fix property not existing error.
  ...
  protected onClose(e: CloseEvent) {
    ...
    console.log('🔌 Connection closed', { code: e.code, reason, wasClean: e.wasClean });
    this.log(
      `server.${e.type}`,
